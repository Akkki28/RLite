...
class DQN(nn.Module):
    def __init__(self, state_size, action_size, gamma=0.95, epsilon=1.0, epsilon_decay=0.995, 
                 epsilon_min=0.01, learning_rate=0.001, n_layers=24):
    ...
    def act(self, state):
        ...
        return T.argmax(act_values).item()
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            target = reward
            if not done:
                target = reward + self.gamma * T.max(self.model(next_state)).item()
    ...
    def fit(self, env, n_episodes=1000, batch_size=32, t_max=200, penalty=-10):
        ...
        for e in range(n_episodes):
            ...
            if e % 100 == 0:
                self.save(os.path.join(output_dir, f"weights_{e:04d}.pth"))
    ...
    def fit(self, env, n_episodes=1000, t_max=2000):
        for i in range(n_episodes):
            ...
            while not done:
                ...
                dist = T.distributions.Categorical(probs=probs)
                action = dist.sample().item()
                new_state, reward, done, _, _ = env.step(action)
                actions.append(T.tensor(action, dtype=T.int))
                states.append(state)
                rewards.append(reward)
                state = T.tensor(new_state, dtype=T.float)
            ...
            for state_, action_, G in zip(states, actions, discounted_returns):
                probs = self.model(state_)
                dist = T.distributions.Categorical(probs=probs)
...
class PPO(nn.Module):
    ...
    def choose_action(self, state):
        ...
        return action.item(), dist_cat.log_prob(action), value
    ...
    def learn(self, states, actions, log_probs, rewards, dones, values, batch_size, n_epochs):
        ...
        for _ in range(n_epochs):
            for i in range(0, len(states), batch_size):
                ...
                dist_cat = Categorical(dist)
                new_log_probs = dist_cat.log_prob(actions_batch)
                ...
                critic_loss = ((returns_batch - critic_value.squeeze()) ** 2).mean()
    ...
    def train_model(self, env, n_games, N, batch_size, n_epochs, alpha):
...
class A3C(nn.Module):
    ...
    def worker(self, name):
        ...
        local_actor_critic = A3C(self.env, (self.pi1.in_features,), self.pi.out_features, 
                                  gamma=self.gamma, lr=self.optimizer.param_groups[0]['lr'],
                                  n_games=self.n_games, t_max=self.t_max)
